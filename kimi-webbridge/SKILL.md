...
Use `find_tab` when the user explicitly asks to operate on an already-open tab. It matches by domain, so any URL on the same site works.

```bash
# User says "打开 Kimi" / "去 Kimi" — reuse leftmost matching tab
curl -s -X POST http://127.0.0.1:10086/command \
  -d '{"action":"find_tab","args":{"url":"https://www.kimi.com"},"session":"kimi"}'

# User says "用我打开的 Kimi" / "在我当前的 Kimi 页面上" — prefer active tab
curl -s -X POST http://127.0.0.1:10086/command \
  -d '{"action":"find_tab","args":{"url":"https://www.kimi.com","active":true},"session":"kimi"}'
```

If `find_tab` returns an error containing **"Please update the Kimi WebBridge extension"**, tell the user:

> 请更新 Kimi WebBridge 浏览器扩展后重试：https://kimi.com/features/webbridge

Do not retry the failed tool after showing this message.
